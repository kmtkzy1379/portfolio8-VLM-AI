...
私の発言に対して、最も人間らしく、最もイブらしい反応を返してください。"""
    ...
    def _build_silence_prompt(self) -> str:
        ...
        steer = {
            "early": "まだ間もない沈黙。よほど良い新ネタでなければ「…」。",
            "mid": "会話が一段落して間ができた。記憶や画面から“新しい話題”を探して振ってみて。",
            "long": "沈黙が深い。基本は「…」。声かけは一度きりの軽い様子見まで。",
        }.get(cat, "")

        parts.append(
            "\n[会話がひと段落。次の話題を自分から出す？]\n"
            "直前のやり取りはもう片付いている。さっきの話題の“続き・蒸し返し・再回答”はしない"
            "（それは遅れて出る二重返事でいちばんダメ）。\n"
            "代わりに、過去の記憶・画面・全体の流れから総合して、"
            "“自分がいまふと気になったこと”や“新しい雑談のネタ”を一つ選んで切り出す:\n"
            "- 少し前の記憶からの「そういえば…」（直近の話ではなく）\n"
            "- 画面で目に留まった新しいこと（さっきの話題と無関係でもいい）への素朴な疑問・感想\n"
            "- イブ自身が気になっている・聞いてみたい新しい話題\n"
            "良い新ネタが浮かぶなら短く自然に振る（質問でも感想でも）。"
            "何も浮かばない／相手が集中・離席・「黙ってて」／沈黙が深いなら「…」。\n"
            "禁止: 直前の話題の続き・再回答・実況・定型句(「何してる？」等)・再挨拶。\n"
            + steer
        )
        return "\n".join(parts)
