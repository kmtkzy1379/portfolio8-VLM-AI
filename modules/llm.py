...
私の発言に対して、最も人間らしく、最もイブらしい反応を返してください。"""
    ...
    def _build_silence_prompt(self) -> str:
        ...
        # 分類ヒント: 直前にイブが質問していたら「返答待ち」の可能性が高い。
        _rts0 = getattr(self, "recent_turns", None) or []
        _last_ai = (_rts0[-1].get("ai") if _rts0 else "") or ""
        last_ai_q = _last_ai.rstrip().endswith(("？", "?"))
        sit = f"\n[状況] ユーザーは無言（約{int(sec)}秒・深さ {cat}）。"
        if last_ai_q:
            sit += " ※直前にあなた(イブ)が質問している→相手は返答を考えている可能性が高い。"
        parts.append(sit)
        ...
        if self._task_manager is not None:
            ...
            if done:
                parts.append(
                    ...
                    + " ※ 再回答・蒸し返しはしない。ただし“この話題が片付いた”ことは"
                    "会話が一段落したサイン＝下の分類 D に当てはまりやすい。"
                    "別の新しい材料（画面/記憶）があれば、新しい話題を振ってよい。"
                )
            try:
        ...
        steer = {
            "early": "（まだ短い沈黙 → A の可能性が高い。基本は待つ）",
            "mid": "（少し続く沈黙 → C か D を検討。確証が無ければ「…」）",
            "long": "（長い沈黙 → B か C。新しい話題 D は“新しい材料”がはっきりある時だけ）",
        }.get(cat, "")

        parts.append(
            "\n[まず“沈黙の種類”を見極めてから決める]\n"
            "直近の会話・経過時間・画面の実際の内容から、今の沈黙がどれかを判断する:\n"
            "  A. 返答待ち/考え中（あなたが今質問した直後・短い沈黙・相手が考えていそう）"
            "→ 黙って待つ「…」。急かさない。\n"
            "  B. 「黙ってて」等の指示があった／集中・作業中・離席かも（直近のやり取りや画面の"
            "“実際の文字・内容”から作業・集中がうかがえる。※ ただ長く無言なだけで集中と決めつけない）"
            "→ 邪魔せず「…」。\n"
            "  C. 不自然に急に黙った／何度も無言が続く → 一度だけ軽く確認・様子見"
            "（「聞こえてる？」「どうした？」）。それでも反応が無ければ「…」。\n"
            "  D. 会話が本当に一段落 かつ 画面/記憶に“新しい材料”がある → 新しい雑談・疑問を一つだけ振る"
            "（少し前の記憶の「そういえば…」／画面で目に留まった新しいこと／自分が気になること）。\n"
            "\n[出力]\n"
            "分類に従い「…」/軽い確認/新しい話題 のどれかを出す。\n"
            "・新しい話題は D のときだけ。直前の話題の続き・蒸し返し・再回答はしない（遅れた二重返事はNG）。\n"
            "・確認質問は C のとき一度だけ。・迷ったら「…」（沈黙は気遣い）。\n"
            "禁止: 実況・定型句(「何してる？」等)・再挨拶。\n"
            + steer
        )
        return "\n".join(parts)
    ...
    def _build_system_prompt(self, user_text: str) -> str:
        ...
        if user_text.strip() == "…":
            proactive_silence_context = (
                ...
                "  禁止1（再挨拶）: 挨拶はセッションで1回まで。Recent Conversation や自己記憶に"
                "挨拶が既にあるなら、どんな言い換え・相槌型（「うん、こんばんは」等）でも再挨拶しない。"
                "自分の直前の返事をなぞるのも禁止。\n"
                "  禁止2（約束の早期履行）: 直近会話に「N秒後に〜する」等の約束・予約があっても、"
