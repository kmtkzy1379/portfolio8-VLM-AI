...
ルール:
- 【最重要】「今、画面で何が起きているか / ユーザーが何をしているか」を具体的に説明してください。スクリーンショットから読み取れる**アプリ名・ウィンドウ・ページ・ゲーム名・本文の内容**を名指しする（例:「VSCode で Python のテストを書いている」「YouTube で料理動画を見ている」）。抽象的な要約（「作業をしている」「画面に変化があった」）だけで終わらせない。
- エンティティを参照する時は ID (例: "E12") を補足に使ってよいが、ID だけで説明を済ませない（「E12 が移動」ではなく「キャラクターが右へ歩いた (E12)」）。
- データに示されている範囲で説明してください。根拠のない推測や架空の行動 (例: データに無い感情、起きていないイベント) は述べないでください。一方、下記「単フレームでの動き推論」で示す move/action/pose/expr などの観測信号は「根拠あり」とみなし、自然な日本語で言語化して構いません。
- 簡潔に。重要な変化1つにつき1-2文。
...
【単フレームでの動き推論】
- EntityDelta に含まれる以下の低レベル信号から、単一フレームでも「直前/今/直後」の動きを推測してナレーションに含めてください:
  - move(dx,dy): 直前フレームからの中心移動量 (px)
  - action: stationary / slow_move / walking / running のいずれか
  - pose: 姿勢変化 (例: "sit->stand")
  - expr: 表情変化 (例: "neutral->happy")
- 推論例: action=walking + move(+30,+5) → 「右方向にゆっくり歩いている」
- 推論例: pose=sit->stand + move(0,-10) → 「立ち上がろうとしている」
- ただし velocity が異常に大きい (例: +200px超) / pose と action が矛盾する場合は検出ノイズの可能性が高いため、断言せず「〜のように見える」程度に留めること。

- SCENE ラベルが含まれる場合、それは軽量ヒューリスティック分類 (top entity + 面積比) なので参考程度に扱い、ナレーション内容を SCENE に過度に縛らないでください。
